...
class ChatSection(QWidget):
    def __init__(self):
        ...
        movie = QMovie(GraphicsDirectoryPath('Arvin.gif'))
        max_gif_size_W = 480
        max_gif_size_H = 270
...
class CustomTopBar(QWidget):
    ...
    def showMessagedesktop(self):
        if self.current_desktop is not None:
            self.current_desktop.hide()

        message_desktop = Messagedesktop(self)
        layout = self.parent().layout()
        if layout is not None:
            layout.addWidget(message_desktop)
        self.current_desktop = message_desktop

    def showInitialdesktop(self):
        if self.current_desktop is not None:
            self.current_desktop.hide()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        top_bar = CustomTopBar(stacked_widget , self)
